...
import { getOrdersBySellerId, getOrdersByStoreId } from "@/actions/orders.actions";
import OrdersDetailsDialog from "./orders-details-dialog";
import { Badge } from "@/components/ui/badge";

interface OrdersProps {
  order: Awaited<ReturnType<typeof getOrdersBySellerId>>[0];
}
...
export const ordersColumns: ColumnDef<OrdersProps["order"]>[] = [
  ...
  },
  {
    ...
    header: "Confirmation",
    cell: ({ row }) => {
      const confirmationStatus = row.original!.confirmationStatus.toString();
      return (
        <div>
          <span className="block text-left w-[100px]">
            <Badge
              variant={
                confirmationStatus === "PENDING" ? "default" :
